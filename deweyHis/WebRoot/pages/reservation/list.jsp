...
<SCRIPT language="javascript">
	function query() {
		form1.action='<c:url value="/reserveList/list"/>';
		form1.submit();
	}
	function query1() {
		form1.action='<c:url value="/reserveList/list1"/>';
		form1.submit();
	}
	function expExcel() {
		form1.action='<c:url value="/reserveList/listExp"/>';
		form1.submit();
	}
...
<table align="center" class="result" border="1" bordercolordark="#ffffff" bordercolorlight="#eeeeee" cellpadding="3" cellspacing="0" style="border-collapse:collapse;">
	  <tbody>
		  <tr class="hcititle">
		    	<td align=left colSpan=8>【旅客预订查询】</td>
		  </tr>
		  <tr>
		  	<td>姓名</td>
  	...
  	<thead>
		...
		<c:choose>
			<c:when test="${empty pageList}">
				<tr>
					<td colspan="17" align="center">无预订信息</td>
				</tr>
			</c:when>
			<c:otherwise>
				<c:forEach var="rs" items="${pageList}" varStatus="vStatus">
				  <tr> 
						...
						<td align="center" >
						    <input  type="button" onclick="javascript:checkInShow(<c:out value="${rs.checkIn.id}"/>);"  value="查看" />&nbsp;
							<input  type="button" <c:if test="${rs.checkIn.status!='在住'}">disabled</c:if> onclick="javascript:checkInEdit(<c:out value="${rs.checkIn.id}"/>);"  value="编辑" />&nbsp;
							<input  type="button" <c:if test="${rs.checkIn.status!='在住'}">disabled</c:if> onclick="javascript:custCheckInConsumeList(<c:out value="${rs.id}"/>,<c:out value="${rs.checkIn.id}"/>,'<c:out value="${rs.roomNo}"/>');"  value="取消预订" />&nbsp;
							<input  type="button" <c:if test="${rs.checkIn.status!='在住'}">disabled</c:if> onclick="javascript:foregiftList(<c:out value="${rs.id}"/>,<c:out value="${rs.checkIn.id}"/>,'<c:out value="${rs.roomNo}"/>');"  value="预订入住" />
						</td>
				  </tr>
			  </c:forEach>
				<tr>
