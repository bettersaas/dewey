...
function formSubmit()
{
	...
	form1.action="${pageContext.request.contextPath}/contractualUnit/update";
	form1.target="_self";
	form1.submit();
}
</script>
...
<BODY>
  <form name="form1" method=post action="<c:url value="/contractualUnit/update"/>" target="_self">
  		<input type="hidden" name="id"  value="<c:out value="${contractualUnit.id}"/>">
		...
		<div align="center">
		  <TABLE class="hci" border="1" bordercolordark="#ffffff" bordercolorlight="#eeeeee" cellpadding="3" cellspacing="0">
		    <TBODY>
		    <TR class="hcititle">
		      <TD colSpan="4">【编辑协议单位】</TD>
		    </TR>
			<tr>
				<td class="hcihead">单位名称</td>
				<td class=hcicontent colspan="3"><input type="text" name="company"   maxlength="40" value="<c:out value="${contractualUnit.company}"/>"></td>
			</tr>
			<tr>
				<td class="hcihead">联系人</td>
				<td class=hcicontent colspan="3"><input type="text" name="linkman"   maxlength="40" value="<c:out value="${contractualUnit.linkman}"/>"></td>
			</tr>
			<tr>
				<td class="hcihead">联系地址</td>
				<td class=hcicontent colspan="3"><input type="text" name="address"   maxlength="120" value="<c:out value="${contractualUnit.address}"/>"></td>
			</tr>
			<tr>
				<td class="hcihead">联系电话</td>
				<td class=hcicontent colspan="3"><input type="text" name="telphone"   maxlength="50" value="<c:out value="${contractualUnit.telphone}"/>"></td>
			</tr>
			<tr>
				<td class="hcihead">传真</td>
				<td class=hcicontent colspan="3"><input type="text" name="fax"   maxlength="20" value="<c:out value="${contractualUnit.fax}"/>"></td>
			</tr>
			<tr>
				<td class="hcihead">备注</td>
				<td class=hcicontent colspan="3"><textarea   name="remark"  cols="80"   rows="5"><c:out value="${contractualUnit.remark}"/></textarea></td>
			</tr>
			<tr>
				<td class=hcicontent colspan="4">
					<table  width="60%">
						...
						<c:forEach var="rs" items="${contractualUnit.merContractualRelations}" varStatus="vStatus">
							<tr>
								<input type="hidden" name="merContractualRelations[<c:out value="${vStatus.index}"/>].id"                    value="<c:out value="${rs.id}"/>">
								...
								<td><input type="text" name="merContractualRelations[<c:out value="${vStatus.index}"/>].contractualPrice" value="<c:out value="${rs.contractualPrice}"/>"/></td>
							</tr>
						</c:forEach>
					</table>
				</td>
