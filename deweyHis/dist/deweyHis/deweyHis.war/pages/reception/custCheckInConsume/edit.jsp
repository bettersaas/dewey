...
function formSubmit()
{
	...
	form1.action="${pageContext.request.contextPath}/custCheckInConsume/update";
	form1.target="_self";
	form1.submit();
}
function setTotal()
...
<BODY>
  <form name="form1" method=post action="<c:url value="/custCheckInConsume/update"/>" target="_self">
  		<input type="hidden" name="id"         value="<c:out value="${custCheckInConsume.id}"/>">
        ...
        <input type="hidden" name="merServices.id"            value="<c:out value="${custCheckInConsume.merServices.id}"/>">
		  ...
		  <TABLE class="hci" border="1" bordercolordark="#ffffff" bordercolorlight="#eeeeee" cellpadding="3" cellspacing="0">
			...
			<tr>
				...
				<td class=hcicontent colspan="3">
				    ...
				    <c:forEach var="rs" items="${merServicesList}" varStatus="vStatus">
				    		<c:if test="${custCheckInConsume.merServices.id == rs.id}">value="<c:out value="${rs.name}"/>"</c:if>
				    </c:forEach>
				    >
				</td>
			</tr>
			<tr>
				...
				<td class=hcicontent colspan="3"><input type="text" name="merServicesPrice" disabled
				    <c:forEach var="rs" items="${merServicesList}" varStatus="vStatus">
				    		<c:if test="${custCheckInConsume.merServices.id == rs.id}">value="<c:out value="${rs.price}"/>"</c:if>
				    </c:forEach>
				></td>
			</tr>
			<tr>
				<td class="hcihead">数量</td>
				<td class=hcicontent colspan="3"><input type="text" name="scalar"   maxlength="10"  value="<c:out value="${custCheckInConsume.scalar}"/>"  onBlur="setTotal();"></td>
			</tr>
			<tr>
				<td class="hcihead">单位</td>
				<td class=hcicontent colspan="3"><input type="text" name="merServicesConsumeUnit" disabled
				    <c:forEach var="rs" items="${merServicesList}" varStatus="vStatus">
				    		<c:if test="${custCheckInConsume.merServices.id == rs.id}">value="<c:out value="${rs.consumeUnit}"/>"</c:if>
				    </c:forEach>
				></td>
			</tr>
			<tr>
				<td class="hcihead">金额</td>
				<td class=hcicontent colspan="3"><input type="text" name="total"   maxlength="10" value="<c:out value="${custCheckInConsume.total}"/>" onBlur="setTotal();"></td>
			</tr>
			<TR>
