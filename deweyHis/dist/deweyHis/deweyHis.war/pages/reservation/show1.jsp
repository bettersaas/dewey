		  ...
		  <TABLE class="hci" border="1" bordercolordark="#ffffff" bordercolorlight="#eeeeee" cellpadding="3" cellspacing="0">
		   <THEAD> 
		    ...
		    <TBODY> 
					...
					<td colspan="8">
						<table  id="custCheckInRoomList"  width="100%"  style="border-collapse:collapse;"  border="1" align="center" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td height="25" width="*" align="center">备注(*双击修改)</td>
				    		</tr>
				...
				<tr>
					<td colspan="8">
						<div class="footlefthead" align="center">消费列表</div>
						<div class="footrighthead" align="center"> 押金列表</div>
						<div class="footleft">
							<!-- 显示每个入住客房的消费列表信息 -->
							...
							<tbody  <c:choose><c:when  test="${vStatus.first}">style="display:block;"</c:when><c:otherwise>style="display:none;"</c:otherwise></c:choose>   >
								...
								<tr>
								 ...
								 <c:choose>
									<c:when test="${empty rs.custCheckInConsumes}">
										<tr>
											<td height="24"  colspan="6" align="center">无消费信息</td>
										</tr>
									</c:when>
									<c:otherwise>
										<c:forEach var="rs1" items="${rs.custCheckInConsumes}" varStatus="vStatus">
										  <tr> 
												<td height="24"  style="border-bottom:1px solid #e1e1e1;height:28px;padding-left:6px;" align="center"><c:out value="${vStatus.index+1}"/></td>
												<td align="center"><c:out value="${rs1.merServices.name}"/></td>
												<td align="center"><c:out value="${rs1.merServices.price}"/></td>
												<td align="center"><c:out value="${rs1.scalar}"/></td>
												<td align="center"><c:out value="${rs1.merServices.consumeUnit}"/></td>
												<td align="center"><c:out value="${rs1.total}"/></td>
										  </tr>
									  </c:forEach>
									</c:otherwise>
								 </c:choose>
					     ...
					     <div class="footright">
							...
							<tbody  <c:choose><c:when  test="${vStatus.first}">style="display:block;"</c:when><c:otherwise>style="display:none;"</c:otherwise></c:choose>   >
								...
								<tr>
								 ...
								 <c:choose>
									<c:when test="${empty rs.foregifts}">
										<tr>
											<td height="24"  colspan="6" align="center">无消费信息</td>
										</tr>
									</c:when>
									<c:otherwise>
										<c:forEach var="rs1" items="${rs.foregifts}" varStatus="vStatus">
										  <tr> 
												<td height="24"  style="border-bottom:1px solid #e1e1e1;height:28px;padding-left:6px;" align="center"><c:out value="${vStatus.index+1}"/></td>
												<td align="center"><c:out value="${rs1.roomNo}"/></td>
												<td align="center"><c:out value="${rs1.payAmount}"/></td>
												<td align="center"><c:out value="${rs1.payType}"/></td>
												<td align="center"><fmt:formatDate   value="${rs1.lcd}" pattern="yyyy-MM-dd HH:mm" /></td>
												<td align="center"><c:out value="${rs1.merUserName}"/></td>
										  </tr>
									  </c:forEach>
									</c:otherwise>
								 </c:choose>
